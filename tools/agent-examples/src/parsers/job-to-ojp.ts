 ...
 * Does NOT call an LLM. Instead, it returns a partially-filled OJP document
 ...
 *   1. Call parseJobPosting({ text, sourceUrl }) to get the skeleton and field map
 *   2. Fill in the document fields using its own extraction from the text
 *   3. Call validateJobPosting({ document }) to validate the result
 *   4. Return the completed OJP document to the user
 *
 * MCP tool name: ojp_parse_job_posting
 */

import { validateJobPosting } from "../ojp-tools.js";
...
export interface ParseJobPostingInput {
  /** Raw job posting text (plain text, markdown, or extracted HTML text). */
  text: string;
  /** Source URL of the job posting, if known. */
  sourceUrl?: string;
}

export interface FieldConfidence {
  /** JSON Pointer path in the OJP document (e.g. '/role/title'). */
  path: string;
...
export interface ParseJobPostingResult {
  /**
   * A partially-filled OJP document skeleton.
  ...
  /**
   * List of fields commonly missing from job postings and requiring
   * explicit research or inference (e.g. exact salary range, team size).
   */
  gaps: string[];
  /** Whether the skeleton passes OJP schema validation (always false until populated). */
  valid: boolean;
...
/**
 * Produces a structured OJP extraction template from raw job posting text.
 *
 * MCP tool name suggestion: "ojp_parse_job_posting"
 * MCP tool description: "Produce a structured OJP extraction template from raw job posting text.
 * Returns a document skeleton with field annotations, confidence levels, and a list of
 * fields commonly missing from job postings. The calling agent should use this as a guide
 * to extract and populate the full OJP document."
 */
export function parseJobPosting(input: ParseJobPostingInput): ParseJobPostingResult {
  const { text, sourceUrl } = input;

  // Build a skeleton OJP document with extraction annotations
  const skeleton: Record<string, unknown> = {
    ...
    role: {
      ...
      seniority:
        "_EXTRACT_one_of_intern_junior_mid_senior_staff_principal_lead_director_vp_c_level_or_null",
    ...
    requirements: {
      must_have: {
        skills:
          "_EXTRACT_array_of_required_skills_with_name_and_min_years",
        experience_years: {
          min: "_EXTRACT_minimum_years_of_total_experience_or_null",
        },
        credentials: "_EXTRACT_array_of_required_degrees_or_empty",
        certifications: "_EXTRACT_array_of_required_certifications_or_empty",
        languages:
          "_EXTRACT_array_of_required_languages_with_code_and_cefr_level_or_empty",
        legal: {
          work_authorization:
            "_EXTRACT_work_authorization_requirements_or_null",
        },
      },
      nice_to_have: {
        skills: "_EXTRACT_array_of_preferred_skills_with_name_and_optional_min_years",
        experience_in_place_of_education:
          "_EXTRACT_boolean_whether_experience_accepted_instead_of_degree_or_null",
    ...
    offering: {
      compensation: {
        salary: {
          ...
          currency:
            "_EXTRACT_iso4217_currency_code_eg_EUR_USD_or_null",
          period:
            "_EXTRACT_one_of_annual_monthly_daily_hourly_or_null",
        },
        additional_compensation:
          "_EXTRACT_array_of_bonus_equity_perks_strings_or_empty",
        transparency:
          "_EXTRACT_one_of_public_on_request_after_interview_or_null",
      },
      location: {
        arrangement:
          "_EXTRACT_one_of_remote_hybrid_onsite",
  ...
  const fieldConfidence: FieldConfidence[] = [
    // High confidence — reliably extractable from standard job postings
    {
      path: "/role/title",
      confidence: "high",
      reason: "Job title is always explicitly stated in the posting title or header.",
    },
    {
      path: "/role/description",
      confidence: "high",
      reason: "Full role description is the main body of the posting.",
    },
    {
      path: "/role/employment_type",
      confidence: "high",
      reason: "Employment type (full-time, contract, etc.) is typically stated explicitly.",
    },
    {
      path: "/role/responsibilities",
      confidence: "high",
      reason: "Responsibilities are listed as bullet points in nearly all postings.",
    },
    {
      path: "/organization/name",
      confidence: "high",
      reason: "Company name is always present.",
    },
    {
      path: "/requirements/must_have/skills",
      confidence: "high",
      reason: "Required skills are listed in 'requirements' or 'what you'll need' sections.",
    },
    // Medium confidence — present but require interpretation
    {
      path: "/role/seniority",
      confidence: "medium",
      reason: "Infer from title (Senior, Lead, Junior) and experience requirements.",
    },
    {
      path: "/offering/location/arrangement",
      confidence: "medium",
      reason: "Remote/hybrid/onsite is usually stated but terminology varies.",
    },
    {
      path: "/organization/size",
      confidence: "medium",
      reason: "Company size bracket requires mapping from headcount or funding stage.",
    },
    {
      path: "/offering/benefits",
      confidence: "medium",
      reason: "Benefits are listed in most postings but vary in detail.",
    },
    {
      path: "/process/stages",
      confidence: "medium",
      reason: "Interview stages are sometimes described but often omitted.",
    },
    // Low confidence — commonly missing from job postings
    {
      path: "/offering/compensation/salary",
      confidence: "low",
      reason: "Salary ranges are omitted in ~60% of postings, especially outside Germany/Austria.",
    },
    {
      path: "/team/size",
      confidence: "low",
      reason: "Team headcount is rarely stated explicitly.",
    },
    {
      path: "/process/total_duration_days",
      confidence: "low",
      reason: "Total hiring timeline is rarely quantified.",
    },
    {
      path: "/offering/location/visa_sponsorship",
      confidence: "low",
      reason: "Visa sponsorship policy is often ambiguous or missing.",
    },
    {
      path: "/culture/values",
      confidence: "low",
      reason: "Company values may appear on career pages but not in the posting itself.",
    },
  ];

  // Common gaps in job postings
  const gaps: string[] = [
  ...
  // Contextual hints for the calling agent
